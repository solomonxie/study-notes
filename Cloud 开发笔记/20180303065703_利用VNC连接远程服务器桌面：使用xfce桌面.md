...
![image](20180303065703_利用VNC连接远程服务器桌面：使用xfce桌面_files/img_01.png)

4. 第四步：开启服务器端口
服务器端口开放需要在供应商的网页控制面板里调整，各不相同。
因为vnc使用的是从`5900`端口开始，第1个桌面为`5901`，第2个为`5902`以此类推。所以就索性设置为`5900-5999`之间的端口都开放。
下面的是Lightsail的配置：
![image](20180303065703_利用VNC连接远程服务器桌面：使用xfce桌面_files/img_02.png)
...
连接成功：
![image](20180303065703_利用VNC连接远程服务器桌面：使用xfce桌面_files/img_03.png)
